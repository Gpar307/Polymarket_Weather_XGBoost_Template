...
Do not commit large generated files, model artifacts, API keys, or local environment files.
...
# Generated data
*_temperature_orderbooks.csv
historical_predictions_*.csv
historical_predictions_failures_*.csv
today_features_*.csv
today_probabilities_*.csv
backtest_trades_*.csv
backtest_daily_*.csv
backtest_summary_*.txt
*_FINAL_ORACLE_DATA.csv
*_oracle_backup.csv
...
If you want to share an example model output, create a small anonymized sample CSV instead of committing full historical datasets.
...
- Rename `XGBOOST_template (1).ipynb` to `XGBOOST_template.ipynb`.
- Rename `mass_scraper_claude_version.py` to something neutral, for example `oracle_scraper_resumable.py`.
- Move example configs into a `configs/` folder.
- Move scripts into a `src/` or `scripts/` folder.
- Remove personal machine paths, temporary comments, and city-specific hardcoding where possible.
- Keep API keys outside the repository.
- Add a small `sample_data/` folder only if the data can be shared legally.
